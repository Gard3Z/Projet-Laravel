...
<form action="{{ route('team.invite') }}" method="POST" class="space-y-8 w-1/2">
        ...
        <h1>Invite in your team</h1>

        <div>
            <label for="login" class="block text-sm font-medium leading-6 text-gray-900">Pseudo</label>
            <div class="relative mt-2 rounded-md shadow-sm">
                <input type="text" name="login" id="login"
                       class="block w-full rounded-md border-0 py-1.5 pr-10 @error('url') text-red-900 placeholder:text-red-300 ring-red-300 focus:ring-red-500 @enderror ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6"
                          placeholder="Pseudo" value="{{ old('login') }}" required>
                @error('login')
